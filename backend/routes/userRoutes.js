    ...
    const messages = [
      {
        role: "system",
        content:
          "You are a medical assistant. Provide possible causes for the given symptoms and suggest whether to seek medical attention or manage at home.",
      },
      {
        role: "user",
        content: `The patient has the following profile: ${JSON.stringify(
          profile
    ...
    const messages = [
      {
        role: "system",
        content:
          "You are a first aid assistant. Provide step-by-step first aid instructions based on the given scenario.",
      },
      {
        role: "user",
        content: `The user needs first aid advice for the following scenario: ${scenario}. Please provide step-by-step instructions.`,
      },
    ];
    const response = await openai.chat.completions.create({
      model: "gpt-3.5-turbo",
